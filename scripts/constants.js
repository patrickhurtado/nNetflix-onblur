...
var huluConstants = {
  ...
  getImage: (card) =>
    card.querySelector(
      `.StandardEmphasisHorizontalTileThumbnail__image-container`
    ),
  getTitle: (card) =>
    card.querySelector(`.StandardEmphasisHorizontalTileContent__title`),
  getDescription: (card) =>
    card.querySelector(`.StandardEmphasisHorizontalTileContent__description`),
  getProgress: (node) => {
    const progressBar = node.querySelector(".StatusBar__progress");
    if (!progressBar) return 0;

    const percentage =
      progressBar.getAttribute("aria-label").split("%")?.[0] || "0";
...
var disneyplusConstants = {
  ...
  getDescription: (card) =>
    card.getElementsByClassName(
      `metadata text-color--secondary text--left`
    )?.[0],
  getProgress: (node) => {
    const progressBar = node.getElementsByTagName("progress")?.[0];
    if (progressBar) {
